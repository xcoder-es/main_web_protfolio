# Mobile administration and provider setup

**Verified:** 2026-06-15

This runbook covers an operational constraint: Carlos must be able to configure and operate the platform from an Android browser without needing a laptop or desktop computer.

It does not limit product compatibility. The public website and administrator dashboard must remain responsive and accessible across mobile phones, tablets, laptops and desktop browsers.

Use the mobile browser's desktop-site mode only when a third-party provider dashboard hides configuration controls on a narrow viewport.
